...
class AnalyzerService:
    ...
    def model_info(self) -> ModelInfoResponse:
        ...
        payload["latest_ingestion_sources"] = self._latest_ingestion_sources()
    ...
    def _latest_ingestion_sources(self) -> dict[str, str]:
        ...
        if manifest_path.exists():
            ...
            if isinstance(manifest, dict):
                ...
                if fetched_at and isinstance(sources, dict):
                    return {
                        str(source): str(fetched_at)
                        for source in sources
                    }

        latest_snapshot_path = self.settings.data_dir / "normalized" / "latest.parquet"
        if latest_snapshot_path.exists():
            fallback = datetime.fromtimestamp(latest_snapshot_path.stat().st_mtime, timezone.utc).isoformat()
